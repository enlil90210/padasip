...
class AdaptiveFilter():
    ...
    def check_array_param(self, param, low, high, name):
        """
        Check if, for each array entry, the value of the given parameter is in the given range
        and a float.
        Designed for testing parameters like `mu` for RLS with multiple forgetting.
        To pass this function the variable `param` must be able to be converted
        into a float with a value between `low` and `high`.

        **Args:**

        * `param` : parameter to check (list or numpy array)
        ...
        * `param` : checked parameter converted to a np array of floats float

        """
        if type(param) == list:
            param = np.array(param)
        else: 
            if not type(param).__module__ == np.__name__:
                raise ValueError(
                    'Parameter {} is not a list or numpy array'.format(name)
                    )
        if low != None or high != None:
            if not ((param >= low).all() and (param <= high).all()):
                raise ValueError('Parameter {} is not in range <{}, {}>'
                    .format(name, low, high))    
        return param 
